...
incr(Name, Key) -> incr(Name, Key, 1).
incr(Name, Key, Amount) -> gen_server:cast(Name, {incr, Key, Amount}).
...
add_subscriber(Name, Ref) -> gen_server:call(Name, {add_subscriber, Ref}).
...
init([Name, ElliController]) ->
  ...
  Options = [ordered_set, named_table, public, {write_concurrency, true}],
...
handle_info(push, #state{subscribers = Subscribers} = State) ->
  ...
  Chunk = iolist_to_binary(["data: ", json:to_binary(Stats), "\n\n"]),
...
incr_counter(Key, Amount) ->
  case get({counter, Key}) of
    undefined -> put({counter, Key}, Amount);
    Count     -> put({counter, Key}, Count + Amount)
  end.

record_timing(Name, Key, Time) -> ets:insert(Name, {os:timestamp(), Key, Time}).

iterate_ets(Now, Name) -> iterate_ets(Now, ets:first(Name), Name, []).
...
iterate_ets(_Now, _Key, _Name, Acc) -> Acc.
